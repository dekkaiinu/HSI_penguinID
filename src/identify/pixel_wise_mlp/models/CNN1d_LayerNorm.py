...
class CNN1d_layerNorm(nn.Module):
    @staticmethod
    def weight_init(m):
    ...
    def __init__(self, input_channels, n_classes, kernel_size=None, pool_size=None, dropout_prob=0.0):
        super(CNN1d_layerNorm, self).__init__()
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.ln4(self.conv4(x)))
        x = self.dropout4(x)
        x = self.pool4(x)

        x = x.view(x.size(0), -1)
        x = torch.relu(self.ln5(self.fc1(x)))
        x = self.dropout5(x)
        x = self.fc2(x)
        return x
